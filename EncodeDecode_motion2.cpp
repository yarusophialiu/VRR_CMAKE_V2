...
void EncodeDecode::onLoad(RenderContext* pRenderContext)
{
    ...
    Properties motionVectorProps = {};


    mpRenderGraph = RenderGraph::create(mpDevice, "EncodeDecode");

    mpRenderGraph->createPass("GBuffer", "GBufferRaster", gBufferProps);
    mpRenderGraph->createPass("MotionVector", "MotionVector", motionVectorProps);
    mpRenderGraph->onResize(getTargetFbo().get());
    mpRenderGraph->setScene(mpScene);

    //mpRenderGraph->addEdge("GBuffer.mvec");
    mpRenderGraph->addEdge("GBuffer.mvec", "MotionVector.motionVecs"); // source texture, output texture
    mpRenderGraph->markOutput("MotionVector.colorOut");
    mpRenderGraph->compile(pRenderContext, log);

    // mpRenderGraph->setInput("MotionVector.colorIn", mpRtOut);
...
//     // std::cout << "mpCamera: (" << currPos.x << ", " << currPos.y << ", " << currPos.z << ")\n";
    ...
    if (mpScene)
    {
        ...
        static float motionMagnitude = 0;
        ...
        mpRenderGraph->execute(pRenderContext);
        pRenderContext->blit(mpRenderGraph->getOutput("MotionVector.colorOut")->getSRV(), pTargetFbo->getRenderTargetView(0));
        ...
        // if (outputReferenceFrames)
        {
            ...
            mpRenderGraph->getOutput("MotionVector.colorOut")->asTexture()->captureToFile(0, 0, szRefOutFilePath, Bitmap::FileFormat::BmpFile, Bitmap::ExportFlags::None, false);
...
void EncodeDecode::registerEncoderInputResources(int width, int height, NV_ENC_BUFFER_FORMAT bufferFormat)
{
    ...
    */
    ...
    auto dx12InputTexture = mpRenderGraph->getOutput("MotionVector.colorOut")->asTexture()->getNativeHandle().as<ID3D12Resource*>();
...
void EncodeDecode::renderRT(RenderContext* pRenderContext, const ref<Fbo>& pTargetFbo, int fCount)
{
  ...
  /*  std::cout << "Number of elements in level 10: " << val.size() << std::endl;
    ...
    // std::cout << "total_velocity " << total_velocity << "\n";
    ...
    // // comment out to save motion vector
...
// log motion vector magnitude for the whole sequence
// call motionvector.ps.slang, not uesful, we postprocess in python
// save as example of how to use renderpass
int runMain(int argc, char** argv)
{
    ...
    // // std::string scenePath = "SunTemple_v4/SunTemple_v4/SunTemple/SunTemple.pyscene"; // BistroInterior  bistro_path3
    // std::string scene = "suntemple_statue";
    // unsigned int speedInput = 1;

    // // std::string scenePath = "Bistro/Bistro/BistroInterior.fbx"; // BistroInterior  bistro_path3
    // // // std::string scenePath = "Bistro/Bistro/bistro_path3.fbx"; // BistroInterior  bistro_path3
    ...
    // // // std::string scenePath = "breakfast_room/breakfastroom2.fbx"; // render 出来镜头是反的, and wall painting is black
