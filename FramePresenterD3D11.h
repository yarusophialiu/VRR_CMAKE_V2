...
// public:
//     FramePresenterD3D11() {
//         // Initialize members or perform any necessary setup
//     }
...
public:
    ...
    */
    ...
    // FramePresenterD3D11(const FramePresenterD3D11&) = delete;

    // // Delete the copy assignment operator
    // FramePresenterD3D11& operator=(const FramePresenterD3D11&) = delete;
    ...
    */
    bool PresentHostFrame(BYTE *pData, int nBytes) {
        ...
        memcpy(mappedTexture.pData, pData, std::min(nWidth * nHeight * 4, nBytes));
    ...
    /*
    preesent a device frame using multithreading and timer-related logic
    */
    bool PresentDeviceFrame(unsigned char *dpBgra, int nPitch, int64_t delay) {
        // using a mutex (mtx) to lock and unlock a critical section to ensure thread=-safe
        mtx.lock(); // protect shared state bReady
        if (!bReady) {
            mtx.unlock();
            return false;
        }
        ...
        // create timer to schedule the PresentRoutine function to be called after a certain delay
        if (!CreateTimerQueueTimer(&hTimer, hTimerQueue,
            (WAITORTIMERCALLBACK)PresentRoutine, this, (DWORD)delay, 0, 0))
        {
            std::cout << "Problem in createtimer" << std::endl;
        }
        // often used in combination with timers to wait for the timer to elapse.
