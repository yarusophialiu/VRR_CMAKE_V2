...
EncodeDecode::EncodeDecode(const SampleAppConfig& config) : SampleApp(config)
{
        ...
        // mpCopyFence = mpDevice->createFence();
        mNInputFenceVal = 0;
        mNOutputFenceVal = 0;
        mNDecodeFenceVal = 0;
        mCopyFenceVal = 0;

        mpD3D12Device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&mpCopyFence));
...
void EncodeDecode::generateSettings()
{
    ...
    std::vector<int> speeds = {2};
    // std::vector<int> bitrates = {1000, 1500, 2000,};
    std::vector<int> bitrates = {1000,};
    // std::vector<int> framerates = {30, 40, 50, 60, 70, 80, 90, 100, 110, 120};
    std::vector<int> framerates = {83};
    ...
    for (int s : speeds)
    {
        for (int b : bitrates)
        {
            for (int f : framerates)
            {
                ...
                {
                    mSettings.emplace_back(s, f, w, h, b);
                }
            }
        }
    }
...
void EncodeDecode::onFrameRender(RenderContext* pRenderContext, const ref<Fbo>& pTargetFbo)
{
    ...
    if (mpScene)
    {
        ...
        InterlockedIncrement(&mCopyFenceVal); // next fence point
        ...
        // // blit (on gpu) from one frame buffer (or texture) to another
        ...
        // CPU blocks until GPU reaches fence value 1.
        ...
        ID3D12CommandQueue* pQueue = pRenderContext->getLowLevelData()->getCommandQueueNativeHandle().as<ID3D12CommandQueue*>();
        pQueue->Signal(mpCopyFence.Get(), mCopyFenceVal); // GPU blit finished → Signal fence to mCopyFenceVal 1

        // Signal the GPU fence (this happens *after* the blit is submitted)
        // pRenderContext->signal(mpCopyFence, mCopyFenceVal);
        // cpuWaitForFencePoint(mpCopyFence->getNativeHandle().as<ID3D12Fence*>(), mCopyFenceVal);

        if (outputEncodedFrames && outputReferenceFrames && (fCount_rt >= 1) && (fCount_rt <= 10))
        {
        ...
        if (!outputEncodedFrames && outputReferenceFrames && (fCount_rt >= 1))
        {
            snprintf(szRefOutFilePath, sizeof(szRefOutFilePath), "%s%d.bmp", refBaseFilePath, fCount_rt);
            mpRenderGraph->getOutput("TAA.colorOut")->asTexture()->captureToFile(0, 0, szRefOutFilePath, Bitmap::FileFormat::BmpFile, Bitmap::ExportFlags::None, false);
        ...
        std::string resultStr = std::to_string(speed * mAnimationTimeSecs);
        getTextRenderer().render(pRenderContext, resultStr, mpRtOutFbo, {20, 20});
        pRenderContext->blit(mpRenderGraph->getOutput("TAA.colorOut")->getSRV(), pTargetFbo->getRenderTargetView(0));
        ...
        if (fCount_rt >= 1)  // 2
        {
            ...
            // std::cout << "mAnimationTimeSecs " << mAnimationTimeSecs << "\n";
            std::cout << "speed * mAnimationTimeSecs " << speed * mAnimationTimeSecs << "\n";
            std::cout << "Frame #: " << fCount_rt
            << "  mAnimationTimeSecs=" << std::fixed << std::setprecision(6)
            << mAnimationTimeSecs << std::endl;

            if (outputEncodedFrames)
            {
                // NVENC  should read the texture after gpu finishes processing blit mPEncoderInputTexture720
                encodeFrameBuffer(mAnimationTimeSecs);
            }
...
void EncodeDecode::registerEncoderInputResources(int width, int height, NV_ENC_BUFFER_FORMAT bufferFormat)
{
    ...
    */
    ...
    regRsrcInputFence.pFence = mpCopyFence.Get();
    regRsrcInputFence.waitValue = mCopyFenceVal; // GPU will wait for the fence to reach mNInputFenceVal（0） before starting the encoding
    ...
    // NVENC signal (signal means raise fence to the new value) when it’s done encoding that same texture,
    InterlockedIncrement(&mCopyFenceVal);
    regRsrcInputFence.signalValue = mCopyFenceVal; // fence will be set to mNInputFenceVal (1) after the GPU finishes its task.
    regRsrcInputFence.bSignal = true;
...
*/
NVENCSTATUS EncodeDecode::encodeFrameBuffer(double timeSecs)
{
    ...
    // before NVENC reads from that texture mVMappedInputBuffers[bufferIndex],
    //  you want to make sure the GPU has finished writing into it via blit()
    ...
    // Before reading this texture, wait for a fence to reach a certain value.
    // NVENC wait mCopyFenceVal becomes 1
    mVInputRsrc[bufferIndex]->inputFencePoint.pFence = mpCopyFence.Get();
    // Wait for the fence to reach this specific value before encoding.
    mVInputRsrc[bufferIndex]->inputFencePoint.waitValue = mCopyFenceVal; // mCopyFenceVal mNInputFenceVal
    // mVInputRsrc[bufferIndex]->inputFencePoint.waitValue = mNInputFenceVal;
    mVInputRsrc[bufferIndex]->inputFencePoint.bWait = true;

    // Signal when NVENC finishes (optional but useful)
    // signal N + 1
    InterlockedIncrement(&mCopyFenceVal); // next value for post‑encode signal
    mVOutputRsrc[0]->pOutputBuffer = outputBuffer;
    mVOutputRsrc[0]->outputFencePoint.pFence        = mpCopyFence.Get();
    mVOutputRsrc[0]->outputFencePoint.signalValue   = mCopyFenceVal;
    ...
    */
    ...
    if (outputEncodedFrames)
    {
        ...
        std::cout << "encodeframebuffer timeSecs: " << speed * timeSecs << std::endl;
...
void EncodeDecode::setResolution(unsigned int width, unsigned int height)
{
    ...
    mpRtOut = getDevice()->createTexture2D(
        width, height, ResourceFormat::BGRA8Unorm, 1, 1, nullptr, ResourceBindFlags::UnorderedAccess | ResourceBindFlags::ShaderResource | ResourceBindFlags::RenderTarget
    );

    std::vector<ref<Texture>> colorTargets;
    colorTargets.push_back(mpRtOut);
    mpRtOutFbo = Fbo::create(getDevice(), colorTargets);
...
void EncodeDecode::renderRT(RenderContext* pRenderContext, const ref<Fbo>& pTargetFbo, uint32_t width, uint32_t height)
{
    ...
    */
    ...
    // After you're done with all the above (decode + RT), signal mpDecodeFence with value 1.
...
int runMain(int argc, char** argv)
{
    ...
    // unsigned int speedInput = std::stoi(argv[3]);

    unsigned int width = 1920; // 1920 1280
    unsigned int height = 1080; // 1080 720
    std::string scene = "bedroom";
    std::string scenePath = "bedroom/path3_seg2.fbx"; // no texture, objects are black
    ...
    // encodeDecode.setSpeed(speedInput); // TODO: only when save reference
